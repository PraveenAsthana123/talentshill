...
export interface VideoAgentResult { runId: string; agentCount: number; steps: AgentStepRecord[]; totalTokensUsed: number; videoId: string | null; recommendation: string | null }
...
// a human editor.
export async function runVideoContentAgent(params: { videoId: string; triggeredBy?: string | null }): Promise<VideoAgentResult> {
  const agentRole = 'video_content_advisor';
  const steps: AgentStepRecord[] = [];
  let totalTokens = 0;
  let stepIndex = 0;

  const runId = logOperationRun({ moduleKey: 'videos', operationName: 'agentic_content_recommendation', executionMode: 'agentic', status: 'running', inputPayload: params, triggeredBy: params.triggeredBy });
  ...
  try {
    const planInput = `Video page "${video.title}" (category: ${video.category || 'none'}, active: ${video.isActive}). Plan how to recommend a content improvement (max 2 steps).`;
    const planResult = await ollamaChat([
      { role: 'system', content: 'You are a video-library content planning agent. Be concise.' },
    ...
    const content = await runVideoContentPipeline({ videoId: params.videoId, triggeredBy: params.triggeredBy });
    ...
    const actInput = `Video: title="${video.title}", hasThumbnail=${!!video.thumbnail}, summaryLength=${(video.summary || '').length}, tags=${(video.tags || []).length}, category=${video.category || 'none'}, duration=${video.duration || 'none'}, content score=${content.score}/100, gaps=${failedChecks.join(', ') || 'none'}. In 1-2 sentences, recommend the top improvement. Never invent facts not given.`;
    const actResult = await ollamaChat([
      { role: 'system', content: 'You are a video-library content advisor. Never invent facts about the video not given to you.' },
    ...
    updateOperationRunStatus(runId, 'completed', { outputPayload: { videoId: params.videoId, score: content.score, recommendation: actResult.content }, tokensUsed: totalTokens });
