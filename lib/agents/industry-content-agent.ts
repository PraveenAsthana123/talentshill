...
export interface IndustryAgentResult { runId: string; agentCount: number; steps: AgentStepRecord[]; totalTokensUsed: number; industryId: string | null; recommendation: string | null }
...
// a human editor.
export async function runIndustryContentAgent(params: { industryId: string; triggeredBy?: string | null }): Promise<IndustryAgentResult> {
  const agentRole = 'industry_content_advisor';
  const steps: AgentStepRecord[] = [];
  let totalTokens = 0;
  let stepIndex = 0;

  const runId = logOperationRun({ moduleKey: 'industries', operationName: 'agentic_content_recommendation', executionMode: 'agentic', status: 'running', inputPayload: params, triggeredBy: params.triggeredBy });

  const industry = getIndustryById(params.industryId);
  if (!industry) {
    updateOperationRunStatus(runId, 'failed', { errorMessage: 'industry not found' });
    return { runId, agentCount: 1, steps, totalTokensUsed: 0, industryId: null, recommendation: null };
  }

  try {
    const planInput = `Industry page "${industry.name}" (active: ${industry.isActive}). Plan how to recommend a content improvement (max 2 steps).`;
    ...
    const content = await runIndustryContentPipeline({ industryId: params.industryId, triggeredBy: params.triggeredBy });
    const searchOutput = `Content score: ${content.score}/100`;
    logStep(runId, stepIndex++, 'search', agentRole, params.industryId, searchOutput, 0);
    steps.push({ phase: 'search', agentRole, input: params.industryId, output: searchOutput, tokensUsed: 0 });

    const actInput = `Industry: name="${industry.name}", hasIcon=${!!industry.icon}, descriptionLength=${(industry.description || '').length}, active=${industry.isActive}, content score=${content.score}/100. In 1-2 sentences, recommend the top improvement for this public-facing page. Never invent facts not given.`;
    const actResult = await ollamaChat([
      { role: 'system', content: 'You are a marketing-content advisor. Never invent facts about the industry not given to you.' },
    ...
    updateOperationRunStatus(runId, 'completed', { outputPayload: { industryId: params.industryId, score: content.score, recommendation: actResult.content }, tokensUsed: totalTokens });
    return { runId, agentCount: 1, steps, totalTokensUsed: totalTokens, industryId: params.industryId, recommendation: actResult.content };
  } catch (err) {
