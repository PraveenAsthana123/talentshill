...
// Module 30 on the Operational Portal 10-tab standard. Manual tab wraps
// the pre-existing read-only catalog table (now with a real Verify
// action, previously an orphaned endpoint with zero UI callers).
// Pipeline scores real registry-row drift (recency, description,
// status/UI consistency); Agentic reuses it as the "search" step for a
// local-LLM advisory recommendation.
export default function ModuleRegistryPage() {
  return (
    <div className={styles.page}>
      <SectionHeader title="Module Registry" subtitle="Real, verified built-vs-partial status per admin module, via Manual, Pipeline (deterministic drift scoring), or Agentic (local LLM) execution modes." />
      <Tabs
        tabs={[
