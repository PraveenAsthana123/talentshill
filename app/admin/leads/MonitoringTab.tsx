...
interface MonitoringData {
  ...
  summary: { totalRuns: number; byMode: Record<string, number>; byStatus: Record<string, number>; totalTokensUsed: number; runningNow: number };
...
export default function MonitoringTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/leads/monitoring/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
