...
interface HistoryPoint { id: string; healthScore: number; jobRunnerScore: number; recentErrorsScore: number; jobFailureRateScore: number; dbSizeScore: number; createdAt: string }
...
export default function DashboardTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/health/dashboard/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
      ...
      <div className={styles.subSection}>
        ...
        {data.history.length === 0 ? <p className={styles.empty}>No snapshots yet — run Pipeline to compute the first one.</p> : (
          <table className={styles.table}>
            ...
            <tbody>{data.history.map((h) => <tr key={h.id}><td>{new Date(h.createdAt).toLocaleString()}</td><td>{h.healthScore}</td><td>{h.jobRunnerScore}</td><td>{h.recentErrorsScore}</td><td>{h.jobFailureRateScore}</td><td>{h.dbSizeScore}</td></tr>)}</tbody>
          </table>
        )}
      </div>
    </div>
