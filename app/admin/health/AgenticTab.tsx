...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=health&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/health/agentic/', { method: 'POST' });
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real system-health pipeline as its &quot;search&quot; step, then drafts a real LLM-written recommendation for the single biggest operational risk — local Ollama (phi4-mini), 1 agent (&quot;system_health_advisor&quot;). Advisory only — never restarts the job runner or clears errors.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
