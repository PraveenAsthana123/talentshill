...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=blog&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/blog/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real readiness pipeline as its &quot;search&quot; step, then drafts a real LLM-written top-improvement suggestion — local Ollama (phi4-mini), 1 agent (&quot;blog_readiness_advisor&quot;). Advisory only — never edits or publishes the post.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
