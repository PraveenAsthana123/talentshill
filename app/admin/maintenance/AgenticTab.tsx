...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=maintenance&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/maintenance/agentic/', { method: 'POST' });
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real enforcement self-test as its &quot;search&quot; step, then drafts a real LLM-written recommendation — local Ollama (phi4-mini), 1 agent (&quot;maintenance_enforcement_advisor&quot;). Advisory only — never toggles maintenance mode itself.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
