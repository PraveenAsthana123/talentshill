...
interface DashboardData {
  kpis: { totalServices: number; active: number; inactive: number; unscored: number; avgContentScore: number; totalRuns: number };
...
export default function DashboardTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/services/dashboard/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
    ...
    <div>
      <div className={styles.subSection}>
        ...
        <div className={styles.vizRow}>
          <div className={styles.vizBox}><span>{data.kpis.totalServices}</span>Total services</div>
