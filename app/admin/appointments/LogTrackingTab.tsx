...
export default function LogTrackingTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/operation-runs/?moduleKey=appointments&limit=200').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then((d) => setRuns(d.runs || [])).catch((e) => setError(String(e)));
  }, []);
          ...
          {runs.map((r) => {
            ...
            return (
              <tr key={r.id}>
                ...
                <td>{duration}</td>
                <td>{r.triggeredBy ? r.triggeredBy.slice(0, 8) : 'customer/system'}</td>
