...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=appointments&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/appointments/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real follow-up urgency pipeline as its &quot;search&quot; step, then drafts a real LLM-written next-step recommendation for staff — local Ollama (phi4-mini), 1 agent (&quot;appointment_followup_advisor&quot;). Advisory only — never confirms, cancels, or contacts anyone.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
