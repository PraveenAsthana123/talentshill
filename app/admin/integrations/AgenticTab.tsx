...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=integrations&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/integrations/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real readiness pipeline as its &quot;search&quot; step, then drafts a real LLM-written recommendation — local Ollama (phi4-mini), 1 agent (&quot;integration_account_readiness_advisor&quot;). Advisory only — never edits credentials or account status.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
