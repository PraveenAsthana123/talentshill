...
interface ConsoleRun { id: string; name: string; type: string; status: string; createdAt: string }
interface MonitoringData {
  ...
  activeConsoleRuns: { items: ConsoleRun[]; note: string };
...
export default function MonitoringTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/runs/monitoring/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
  ...
  return (
    <div>
      <OperationHealthCheck moduleKey="runs" />
      ...
      <div className={styles.subSection}>
        <h4>Currently active runs (the Run Console this module monitors)</h4>
        <p className={styles.empty}>{data.activeConsoleRuns.note}</p>
        <table className={styles.table}>
          <thead><tr><th>Name</th><th>Type</th><th>Status</th><th>Created</th></tr></thead>
          <tbody>{data.activeConsoleRuns.items.map((r) => <tr key={r.id}><td>{r.name}</td><td>{r.type}</td><td><Badge variant="success">{r.status}</Badge></td><td>{new Date(r.createdAt).toLocaleString()}</td></tr>)}</tbody>
        </table>
      </div>
      <div className={styles.subSection}>
