...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=runs&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/runs/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real health pipeline as its &quot;search&quot; step, then drafts a real LLM-written recommendation — local Ollama (phi4-mini), 1 agent (&quot;run_health_advisor&quot;). Advisory only — never force-changes a run&apos;s status (use the Manual tab&apos;s Force Status action deliberately for that).</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
