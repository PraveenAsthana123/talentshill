...
export default function PipelineTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=content_overrides&executionMode=pipeline&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/content-overrides/pipeline/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Score a content override&apos;s real safety — an XSS-pattern check (script/javascript:/onX=/iframe), value-present check, and slug-format check. A disclosed, honest finding while building this: <code>getOverrides(pageSlug)</code> exists but has zero real callers on the public site today — this module persists real data but does not yet change what a visitor sees. The XSS check is still real and valuable independent of that. Writes to the real, previously-unused <code>content_overrides.safety_score</code> field.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
