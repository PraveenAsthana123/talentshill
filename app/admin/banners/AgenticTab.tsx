...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=banners&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/banners/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real health pipeline as its &quot;search&quot; step, then drafts a real LLM-written fix recommendation — local Ollama (phi4-mini), 1 agent (&quot;banner_health_advisor&quot;). Advisory only — never edits or deactivates the banner.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
