...
export default function AdminUsersPage() {
  ...
  const handleCreate = async () => {
    ...
    await fetch('/api/admin/users', {
      ...
      body: JSON.stringify({ name: form.name, email: form.email, password: form.password, roleIds: Array.from(form.roleIds) }),
    });
  ...
  return (
    <div className={styles.page}>
      <SectionHeader label="System" title="User Management" subtitle="Manage admin users and their role assignments." />
              ...
              <div className={styles.roleCheckboxes}>
                {allRoles.map(role => (
                  <label key={role.id} className={styles.roleCheckbox}>
                    <input type="checkbox" checked={form.roleIds.has(role.id)} onChange={() => toggleFormRole(role.id)} />
                    {role.name}
                  </label>
                ))}
              </div>
      ...
      {editingUser && (
        <div className={styles.formCard}>
          ...
          <div className={styles.roleCheckboxes}>
            {allRoles.map(role => (
              <label key={role.id} className={styles.roleCheckbox}>
                <input type="checkbox" checked={editRoleIds.has(role.id)} onChange={() => toggleEditRole(role.id)} />
                {role.name}
              </label>
            ))}
          </div>
          <div className={styles.formActions}>
            ...
            <Button size="sm" onClick={handleSaveEdit}>Save Roles</Button>
          </div>
        </div>
      )}
                ...
                <tr key={user.id}>
                  ...
                  <td>
                    <div className={styles.actions}>
                      ...
                      {user.isActive ? (
                        <button className={styles.deactivateBtn} onClick={() => handleDeactivate(user.id)}>Deactivate</button>
                      ) : (
                        <button className={styles.actionBtn} onClick={() => handleActivate(user.id)}>Activate</button>
                      )}
                    </div>
                  </td>
                </tr>
