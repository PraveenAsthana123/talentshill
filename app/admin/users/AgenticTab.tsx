...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=users&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/users/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real security-check pipeline as its &quot;search&quot; step, then drafts a real LLM-written fix recommendation — local Ollama (phi4-mini), 1 agent (&quot;user_security_advisor&quot;). Advisory only — never modifies the account.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
