...
export default function MonitoringTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/users/monitoring/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
  ...
  return (
    <div>
      <OperationHealthCheck moduleKey="users" />
