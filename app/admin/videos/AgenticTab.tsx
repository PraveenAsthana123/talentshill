...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=videos&executionMode=agentic&limit=20')
      .then((r) => (r.ok ? r.json() : { runs: [] })).then((d) => setRuns(d.runs || [])).catch(() => {});
  };

  useEffect(() => {
    fetch('/api/admin/videos').then((r) => (r.ok ? r.json() : { videos: [] }))
      .then((d) => setVideos((d.videos || []).map((v: { id: string; title: string }) => ({ id: v.id, title: v.title }))))
      .catch(() => {});
    loadRuns();
  }, []);
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/videos/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real content-readiness pipeline as its &quot;search&quot; step, then drafts a real LLM-written improvement suggestion — local Ollama (phi4-mini), 1 agent (&quot;video_content_advisor&quot;). Advisory only — never edits the record.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
        <Select label="Video" options={videos.map((v) => ({ value: v.id, label: v.title }))} placeholder="Select a video..." value={videoId} onChange={(e) => setVideoId(e.target.value)} />
