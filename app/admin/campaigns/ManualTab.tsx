...
import sharedStyles from './CampaignsShared.module.css';

interface Campaign {
  id: string; name: string; type: string; status: string; audienceType: string | null;
  subject: string | null; totalSent: number; totalOpened: number; totalClicked: number; createdAt: string;
}
...
export default function ManualTab() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [cRes, lRes, tRes, rRes] = await Promise.all([
        fetch('/api/admin/campaigns/'),
        fetch('/api/admin/lists/'),
        fetch('/api/admin/templates/'),
        fetch('/api/admin/operation-runs/?moduleKey=campaigns&executionMode=manual&limit=20'),
      ]);
  ...
  const handlePause = async (id: string) => {
    await fetch(`/api/admin/campaigns/${id}/`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'paused' }) });
    fetchAll();
  };
  ...
  const statusClass = (s: string) => {
    const map: Record<string, string> = { draft: styles.statusDraft, scheduled: styles.statusScheduled, sending: styles.statusSending, completed: styles.statusCompleted, paused: styles.statusPaused, cancelled: styles.statusCancelled };
  ...
  return (
    <div>
      <div className={sharedStyles.subSection}>
        <h4>Goal</h4>
        <p>Create, launch, and manage real email campaigns end-to-end with full human control.</p>
      </div>
      <div className={sharedStyles.subSection}>
        <h4>Objective</h4>
        <p>No campaign should launch without a real subject, template, non-empty audience, and configured sender profile.</p>
      </div>
      <div className={sharedStyles.subSection}>
        <h4>Checklist (before launch)</h4>
        <ul>
          <li>Run Pipeline readiness check (or verify manually: subject, template, audience, sender profile)</li>
          <li>Optionally run Agentic subject-line suggestions</li>
          <li>Launch, then monitor via the Monitoring tab</li>
        </ul>
      </div>
      ...
      <div className={sharedStyles.subSection}>
        ...
        {!showCreate && <div style={{ marginBottom: 'var(--space-6)' }}><Button size="sm" onClick={() => setShowCreate(true)}>Create Campaign</Button></div>}
      ...
      <div className={sharedStyles.subSection}>
        <h4>Transactional history &amp; status</h4>
