...
import styles from './AdminPresentations.module.css';
...
export default function AdminPresentationsPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ assetType: 'presentation', limit: String(LIMIT), offset: String(offset) });
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/admin/assets', {
        ...
        body: JSON.stringify({ title, assetType: 'presentation', slides: [{ index: 1, title: 'Title Slide', htmlContent: '<h1>Presentation Title</h1>', layout: 'full', notes: '' }] }),
      });
      const data = await res.json();
  ...
  return (
    <div className={styles.page}>
      <SectionHeader label="Marketing" title="Presentations" subtitle="Create and manage slide presentations." />

      <div className={styles.createBar}>
        <input className={styles.input} placeholder="New presentation title..." value={title} onChange={(e) => setTitle(e.target.value)} />
      ...
      {loading ? (
        <div className={styles.empty}>Loading presentations...</div>
      ) : items.length === 0 ? (
        <div className={styles.empty}>No presentations found. Create your first presentation above.</div>
      ) : (
        <div className={styles.grid}>
          {items.map((item) => (
            <Link key={item.id} href={`/admin/content/presentations/${item.id}`} className={styles.card}>
