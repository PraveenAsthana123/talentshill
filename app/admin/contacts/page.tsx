...
export default function AdminContactsPage() {
  ...
  return (
    <div className={styles.page}>
      <SectionHeader label="CRM" title="Contacts" subtitle="Manage your contact database with import and export." />

      <div className={styles.statsRow}>
        <div className={styles.statBadge}><span className={styles.statValue}>{total}</span><span className={styles.statLabel}>Total</span></div>
      </div>
      ...
      {importResult && (
        <div className={styles.importResult}>
          Import complete: {importResult.imported} imported, {importResult.duplicates} duplicates, {importResult.errors} errors
          <button style={{ marginLeft: 'var(--space-3)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)' }} onClick={() => setImportResult(null)}>dismiss</button>
        </div>
      )}
      ...
      <div className={styles.tableWrap}>
        {loading ? <div className={styles.empty}>Loading...</div> : contacts.length === 0 ? <div className={styles.empty}>No contacts found.</div> : (
          <table className={styles.table}>
            <thead><tr><th>Name</th><th>Email</th><th>Company</th><th>Source</th><th>Tags</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {contacts.map(c => (
                <tr key={c.id}>
      ...
      {total > 50 && (
        <div className={styles.pagination}>
          ...
          <span style={{ padding: 'var(--space-2)', fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>{offset + 1}–{Math.min(offset + 50, total)} of {total}</span>
