...
export default function LogTrackingTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/operation-runs/?moduleKey=media&limit=200').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then((d) => setRuns(d.runs || [])).catch((e) => setError(String(e)));
  }, []);
