...
export default function AdminSurveyPage() {
  ...
  const levelColor = (level: string) => {
    switch (level) {
      case 'leader':
        return styles.levelLeader;
      case 'advanced':
        return styles.levelAdvanced;
      case 'developing':
        return styles.levelDeveloping;
      default:
    ...
    <div className={styles.page}>
      <div className="container section">
        ...
        <p className={styles.pageSubtitle}>
          AI Readiness Assessment insights and response data.
        </p>
        ...
        {stats && (
          <>
            <div className={styles.statsGrid}>
              ...
              <div className={styles.statCard}>
                <div className={styles.statValue}>{stats.recentCount30d}</div>
        ...
        <div className={styles.tableWrap}>
          {responses.length === 0 ? (
            <div className={styles.empty}>No survey responses yet.</div>
          ) : (
              ...
              <tbody>
                {responses.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      <span className={cn(styles.levelBadge, levelColor(r.maturityLevel))}>
                        {r.maturityLevel}
                      </span>
                    </td>
                    <td>
                      <div className={styles.tagsWrap}>
                        {r.segmentationTags.slice(0, 3).map((tag) => (
                          <span key={tag} className={styles.tag}>
                            {tag}
                          </span>
                        ))}
                        {r.segmentationTags.length > 3 && (
