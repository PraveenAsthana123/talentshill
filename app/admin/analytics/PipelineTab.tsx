...
export default function PipelineTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=analytics&executionMode=pipeline&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/analytics/pipeline/', { method: 'POST' });
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Score the overall email-program&apos;s real health — open/click/bounce rates scaled against disclosed industry-typical benchmarks (25% open, 3% click, &le;1% bounce), plus contact-base health (active/total ratio) — rolled up from the same Campaigns and Contacts data the Manual tab reads. Each run inserts a new timestamped <code>analytics_snapshots</code> row rather than updating a single entity, since this module has no one record to score.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
