...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=analytics&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/analytics/agentic/', { method: 'POST' });
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real program-health pipeline as its &quot;search&quot; step, then drafts a real LLM-written recommendation for the single biggest lever to improve — local Ollama (phi4-mini), 1 agent (&quot;analytics_priority_advisor&quot;). Advisory only — never edits any campaign or contact record.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
