...
const SUB_TABS = [
  { id: 'resai', label: 'ResAI', content: (<>
    <Section title="Research AI">
      <p>The Agentic tab drafts a content-improvement suggestion from real industry data and the real content-readiness pipeline output — see Agentic/Testing tabs for live-verified evidence.</p>
    </Section>
    <Section title="Responsible AI">
      <p><strong>Data provenance:</strong> the industry record&apos;s own stored fields only. No personal data is involved — industries are public marketing content, not tied to any individual. <strong>Consent:</strong> not applicable.</p>
    </Section>
  </>) },
  ...
  { id: 'govai', label: 'GovAI', content: (
    <Section title="Model, data lineage &amp; approval status">
      <p>Model: phi4-mini:latest, unmodified. Data: the industry record&apos;s own stored fields, read at run time. This module&apos;s admin API (<code>/api/admin/industries</code>) was already correctly RBAC-gated before this build — no security remediation needed. Pilot status — not yet formally approved for unsupervised production use.</p>
    </Section>
  ) },
  ...
  { id: 'decision', label: 'Decision AI', content: (
    <Section title="Decision-rationale logging">
      <p>The Pipeline tab&apos;s per-stage output IS real decision rationale (icon/description/name/active checks shown separately, not just a final number). The Agentic tab&apos;s PLAN step additionally logs the agent&apos;s own stated reasoning.</p>
    </Section>
  ) },
  { id: 'compliance', label: 'Compliance AI', content: (
    <Section title="Regulatory mapping">
      <p>Not applicable — industry records carry no personal data and are public marketing content only.</p>
    </Section>
  ) },
  ...
  { id: 'risk', label: 'Risk AI', content: (<>
    <Section title="Known failure modes">
      <ul>
        ...
        <li>A thin content record (no icon, short description) rendering on the real public page is the main quality risk this pipeline exists to catch.</li>
      </ul>
    </Section>
