...
interface DashboardData {
  kpis: { totalIndustries: number; active: number; inactive: number; unscored: number; avgContentScore: number; missingIcon: number; totalRuns: number };
}
...
export default function DashboardTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/industries/dashboard/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
    ...
    <div>
      <div className={styles.subSection}>
        ...
        <div className={styles.vizRow}>
          <div className={styles.vizBox}><span>{data.kpis.totalIndustries}</span>Total industries</div>
          ...
          <div className={styles.vizBox}><span>{data.kpis.missingIcon}</span>Missing icon</div>
