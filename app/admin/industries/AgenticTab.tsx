...
export default function AgenticTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=industries&executionMode=agentic&limit=20')
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/industries/agentic/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Reuses the real content-readiness pipeline as its &quot;search&quot; step, then drafts a real LLM-written improvement suggestion — local Ollama (phi4-mini), 1 agent (&quot;industry_content_advisor&quot;). Advisory only — never edits the record.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
