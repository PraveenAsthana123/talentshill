...
export default function PipelineTab() {
  ...
  const loadRuns = () => {
    fetch('/api/admin/operation-runs/?moduleKey=email_profiles&executionMode=pipeline&limit=20')
      .then((r) => (r.ok ? r.json() : { runs: [] })).then((d) => setRuns(d.runs || [])).catch(() => {});
  };

  useEffect(() => {
    fetch('/api/admin/email-profiles').then((r) => (r.ok ? r.json() : { profiles: [] }))
      .then((d) => setProfiles((d.profiles || []).map((p: { id: string; name: string }) => ({ id: p.id, name: p.name }))))
      .catch(() => {});
    loadRuns();
  }, []);
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/admin/email-profiles/pipeline/', {
  ...
  return (
    <div>
      <div className={styles.subSection}>
        ...
        <p>Score an email profile&apos;s real send-readiness — fromEmail/replyTo format, signature safety, active status, and (worth 35 of 100 points) whether a real SMTP config is linked. Catches a real, confirmed gap: a profile with no linked SMTP config silently falls back to process.env vars at send time (confirmed live while building the Compose module). Writes to the real, previously-unused <code>email_profiles.readiness_score</code> field.</p>
      </div>
      <div className={styles.subSection}>
        <h4>Input</h4>
        <Select label="Profile" options={profiles.map((p) => ({ value: p.id, label: p.name }))} placeholder="Select a profile..." value={profileId} onChange={(e) => setProfileId(e.target.value)} />
