...
interface PipelineRun { id: string; type: string; status: string; startedAt: string | null; completedAt: string | null }
interface MonitoringData {
  ...
  pipelineRuns: { items: PipelineRun[]; total: number; note: string };
...
export default function MonitoringTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/rag/monitoring/').then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); }).then(setData).catch((e) => setError(String(e)));
  }, []);
  ...
  return (
    <div>
      <OperationHealthCheck moduleKey="rag" />
      ...
      <div className={styles.subSection}>
        <h4>Real job-pipeline runs (rag_runs, distinct from operation_run)</h4>
        <p className={styles.empty}>{data.pipelineRuns.note} Total: {data.pipelineRuns.total}.</p>
        <table className={styles.table}>
          <thead><tr><th>Type</th><th>Status</th><th>Started</th><th>Completed</th></tr></thead>
          <tbody>{data.pipelineRuns.items.map((r) => <tr key={r.id}><td>{r.type}</td><td><Badge variant={r.status === 'completed' ? 'success' : r.status === 'failed' ? 'error' : 'warning'}>{r.status}</Badge></td><td>{r.startedAt ? new Date(r.startedAt).toLocaleString() : '—'}</td><td>{r.completedAt ? new Date(r.completedAt).toLocaleString() : '—'}</td></tr>)}</tbody>
        </table>
      </div>
      <div className={styles.subSection}>
