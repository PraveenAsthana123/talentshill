...
export const GET = withPermission('workflows', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'marketing')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
