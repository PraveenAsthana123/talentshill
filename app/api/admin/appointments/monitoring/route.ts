...
export const GET = withPermission('appointments', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'appointments')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
