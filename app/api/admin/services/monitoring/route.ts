...
export const GET = withPermission('services', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'services')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
