...
export const GET = withPermission('contacts', 'read')(async (_request: NextRequest, _context: unknown) => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'contacts')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
