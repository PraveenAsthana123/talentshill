...
export const GET = withPermission('templates', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'templates')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
