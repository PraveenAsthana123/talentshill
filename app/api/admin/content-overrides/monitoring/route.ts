...
export const GET = withPermission('content_overrides', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'content_overrides')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
