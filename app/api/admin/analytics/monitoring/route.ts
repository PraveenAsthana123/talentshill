...
export const GET = withPermission('analytics', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'analytics')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
