...
export const GET = withPermission('settings', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'settings')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
  ...
  return NextResponse.json({
    ...
    jobQueue: { note: 'Setting updates apply synchronously via the real PUT route; no job queue involved.' },
  });
});
