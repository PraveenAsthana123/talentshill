...
export const GET = withPermission('integrations', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'integrations')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
