...
export const GET = withPermission('maintenance', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'maintenance')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
