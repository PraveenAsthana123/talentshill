...
export const GET = withPermission('email_compose', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'email_compose')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
