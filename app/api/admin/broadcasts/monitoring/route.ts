...
export const GET = withPermission('broadcasts', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'broadcasts')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
