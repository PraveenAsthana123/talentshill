...
export const GET = withPermission('industries', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'industries')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
