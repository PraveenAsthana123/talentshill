...
export const GET = withPermission('health', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'module-registry')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
  ...
  return NextResponse.json({
    ...
    jobQueue: { note: 'Drift scoring runs synchronously via operation_run, on demand -- not yet a real scheduled job. Honest gap: the Module Understanding Standard calls for a real scheduled drift watchdog; this module exposes the same check manually/on-demand until that job exists.' },
  });
});
