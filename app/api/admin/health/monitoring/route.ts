...
export const GET = withPermission('health', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'health')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
  ...
  return NextResponse.json({
    ...
    jobQueue: { note: 'This IS the job queue module -- see the Manual tab for the real live queue stats.' },
  });
});
