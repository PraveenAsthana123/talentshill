...
export const GET = withPermission('features', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'features')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
