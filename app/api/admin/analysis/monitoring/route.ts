...
export const GET = withPermission('analysis', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'analysis')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
