...
export const GET = withPermission('email_profiles', 'read')(async () => {
  const runs = db.select().from(schema.operationRun).where(eq(schema.operationRun.moduleKey, 'email_profiles')).orderBy(desc(schema.operationRun.createdAt)).limit(100).all();
